...
CREATE TRIGGER t_insert_item_pedido
AFTER INSERT ON item_pedido
FOR EACH ROW
BEGIN
	UPDATE produto
	SET qt_estoque = qt_estoque - NEW.qt_itens
		, dt_ultima_alteracao = CURRENT_TIMESTAMP
	WHERE cd_produto = NEW.cd_produto
	;

	UPDATE pedido
	SET vl_total_compra = vl_total_compra + (NEW.qt_itens * NEW.vl_unitario)
		, dt_ultima_alteracao = CURRENT_TIMESTAMP
	WHERE cd_pedido = NEW.cd_pedido
	;
END;

CREATE TRIGGER t_delete_item_pedido
AFTER DELETE ON item_pedido
FOR EACH ROW
BEGIN
	UPDATE produto
	SET qt_estoque = qt_estoque + OLD.qt_itens
		, dt_ultima_alteracao = CURRENT_TIMESTAMP
	WHERE cd_produto = OLD.cd_produto
	;

	UPDATE pedido
	SET vl_total_compra = vl_total_compra - (OLD.qt_itens * OLD.vl_unitario)
		, dt_ultima_alteracao = CURRENT_TIMESTAMP
	WHERE cd_pedido = OLD.cd_pedido
	;
END;

CREATE TRIGGER t_update_item_pedido
